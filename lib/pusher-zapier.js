...
var Zap = {
    ...
    /*
      The one and only argument:
        ...
        bundle.action_raw_fields: <object> # before we replace users' variables

        bundle.trigger_meta.label: <string> # trigger service info
        bundle.trigger_meta.logos: <object> # with urls on keys 16x16, 32x32, 64x64, 128x128
    ...
    */
    ...
    var eventData = JSON.parse( bundle.request.data );

    var requestData = {
      name: bundle.action_fields[ 'event' ],
      data: eventData.data,
      channels: [ bundle.action_fields[ 'channel' ] ]
    };

    bundle.request.data = JSON.stringify( requestData );

    var queryString = pusher.createSignedQueryString( bundle.request.method, bundle.request.data, '/apps/' + credentials.appId + '/events' );
